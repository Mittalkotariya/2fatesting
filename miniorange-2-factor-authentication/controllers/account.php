   ...
   function _goto_sign_in_page(){
	...
	function save_success_customer_config($email, $id, $apiKey, $token, $appSecret)
	{
		...
		if ( ! empty( $mo2f_customer_selected_plan ) ) {
			...
			if (get_option('mo2f_planname') == 'website_security_plan') 
			{
				?>
				<script>window.location.href="admin.php?page=mo_2fa_website_security_plan";</script>
				<?php
			}
			elseif (get_option('mo2f_planname') == 'addon_plan') 
			{
