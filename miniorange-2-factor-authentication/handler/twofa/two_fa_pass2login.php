...
class Miniorange_Password_2Factor_Login {
	...
	function inline_save_success_customer_config($email, $id, $apiKey, $token, $appSecret)
	{
		...
		$Mo2fdbQueries->insert_user( $id);
		$Mo2fdbQueries->update_user_details( $id, array(
	...
	function mo2f_inline_complete_otp(){
				...
				if($content['status'] == 'ERROR'){
					...
					if(!($Mo2fdbQueries->get_user_detail('mo2f_configured_2FA_method',$current_user)=='OTP OVER EMAIL')){
						...
						'mo2f_user_phone' =>$_SESSION[ 'mo2f_phone']
							...
							$Mo2fdbQueries->update_user_details( $current_user->ID, array('mo2f_email_otp_registration_status'=>true) );
							
						}	
					unset($_SESSION[ 'mo2f_phone']);
					$Mo2fdbQueries->update_user_details( $current_user->ID, array( 'mo2f_configured_2FA_method' => 'OTP Over SMS' ) );
//					update_user_meta($current_user,'currentMethod','OTP Over SMS'); //current_user = current_user_id
	...
	function mo2f_inline_verify_otp(){
	...
	function mo2f_inline_complete_kba(){
...
//							update_user_meta($current_user->ID, 'currentMethod','Security Questions');
	...
	function mo2f_inline_complete_mobile(){
...
//						) );
						...
						if(MO2F_IS_ONPREM){ //this should not be here
					        ...
					        $issuer=get_site_option('mo2f_GA_account_name', 'miniOrangeAuth');
...
//										$mo2f_second_factor = get_user_meta($user->ID,'currentMethod',true);
		...
		switch ($value) {
			...
			case 'miniorange_inline_complete_mobile':
				$this->mo2f_inline_complete_mobile();
				break;

			case 'mo2f_inline_kba_option':
				$this->mo2f_inline_complete_kba();
				break;

			case 'miniorange_inline_complete_otp_over_sms':
				$this->mo2f_inline_verify_otp();
				break;

			case 'miniorange_inline_complete_otp':
				$this->mo2f_inline_complete_otp();
				break;
	...
	function miniorange_pass2login_form_fields( $mo2fa_login_status = null, $mo2fa_login_message = null, $redirect_to = null, $qrCode = null, $session_id_encrypt  ) {
		...
		$user_id = $this->mo2f_userID ? $this->mo2f_userID : MO2f_Utility::mo2f_retrieve_user_temp_values( 'mo2f_current_user_id',$session_id_encrypt );
			...
			case 'MO_2_FACTOR_PROMPT_USER_FOR_2FA_METHODS':
				$user_id = $this->mo2f_userID ? $this->mo2f_userID : MO2f_Utility::mo2f_retrieve_user_temp_values( 'mo2f_current_user_id',$session_id_encrypt );
				prompt_user_to_select_2factor_mthod_inline($user_id, $login_status, $login_message,$redirect_to,$session_id_encrypt);
			exit;
			break;
...
//        	$user_email = $Mo2fdbQueries->get_user_detail( 'mo2f_user_email', $current_user->ID );
	...
	function mo2f_pass2login_otp_verification( $user, $mo2f_second_factor, $redirect_to,$session_id=null ) {
		...
		{	
			...
			if(MO2F_IS_ONPREM)
				//$mo2f_user_phone = get_user_meta($user->ID,'email',true);
		...
		if ( $mo2f_second_factor == 'SOFT TOKEN' ) {
			...
			$content = '';
			$response = [];
			if(get_option("cmVtYWluaW5nT1RQ")>0 or $mo2f_second_factor != 'EMAIL')
			{
...
//					$currentMethod = get_user_meta($user->ID,'currentMethod',true);
					...
					$message = $response['message'] . ' You can click on <b>I am locked out</b> link to login via alternate method. or  <a href = " https://login.xecurify.com/moas/login?redirectUrl=https://login.xecurify.com/moas/initializepayment&requestOrigin=otp_recharge_plan">puchase trascactions</a>';
	...
	function miniorange_initiate_2nd_factor( $currentuser, $attributes = null, $redirect_to = null, $otp_token = "",$session_id_encrypt=null ) {
		...
		{
			$dG90YWxVc2Vyc0Nsb3Vk = get_option("dG90YWxVc2Vyc0Nsb3Vk");
		...
		if ( $is_customer_admin ) {
			...
			if ( ($email && $mo_2factor_user_registration_status == 'MO_2_FACTOR_PLUGIN_SETTINGS') or (MO2F_IS_ONPREM and  $mo_2factor_user_registration_status == 'MO_2_FACTOR_PLUGIN_SETTINGS') or (get_option(base64_encode("totalUsersCloud"))<3 and !MO2F_IS_ONPREM) )  { //checking if user has configured any 2nd factor method
				try {
				...
				if ( $mo2f_rba_status['status'] == 'SUCCESS' && $mo2f_rba_status['decision_flag'] ) {
					...
					else if(is_null($mo2f_second_factor) or !isset($mo2f_second_factor) or $mo2f_second_factor == '')
						$mo2f_second_factor = "INLINE";


					$cloud_methods = array("MOBILE AUTHENTICATION","PUSH NOTIFICATIONS","SOFT TOKEN","SMS");
					if(MO2F_IS_ONPREM)
					{	
						$user = $currentuser;
        				...
        				//$user = get_userdatabylogin('admin');
        				 	if(!in_array($mo2f_second_factor, $cloud_methods))
						        $mo2f_second_factor = $Mo2fdbQueries->get_user_detail( 'mo2f_configured_2FA_method', $user->ID );
							...
							else if($mo2f_second_factor != 'Email Verification' && !in_array($mo2f_second_factor, $cloud_methods))
							{
								$mo2f_second_factor = 'INLINE';
						}
						if($flag == 0){
					...
					{	
						...
						if($mo2f_second_factor == 'OTP Over Email')
						{
							$mo2f_second_factor = "EMAIL";
							if(get_option("cmVtYWluaW5nT1RQ")<=0)
							{
								update_option("bGltaXRSZWFjaGVk",1);
						...
						if ( MO2f_Utility::check_if_request_is_from_mobile_device( $_SERVER['HTTP_USER_AGENT'] ) && $kba_configuration_status ) {
							...
							if ( $mo2f_second_factor == 'MOBILE AUTHENTICATION' ) {
								...
								$this->mo2f_pass2login_kba_verification( $currentuser->ID, $redirect_to , $session_id_encrypt );
							}else if ( $mo2f_second_factor == 'INLINE' ) {
							    if(!$exceeded)
...
//								$this->mo2fa_pass2login( $redirect_to, $session_id_encrypt );
	...
	function mo2fa_inline($currentuser,$redirect_to,$session_id){
		...
		//if(MO2F_IS_ONPREM  or 1){
	        //add onprem version
        $Mo2fdbQueries->update_user_details( $currentUserId, array( "mo2f_user_email" => '' ) );
        $Mo2fdbQueries->update_user_details( $currentUserId, array(
            'user_registration_with_miniorange' =>'SUCCESS',
            'mo2f_user_email' =>$email,
            'mo_2factor_user_registration_status' =>'MO_2_FACTOR_INITIALIZE_TWO_FACTOR'
...
//					if(MO2F_IS_ONPREM){
//						if(!in_array($mo2f_configured_2FA_method, $cloud_methods))
