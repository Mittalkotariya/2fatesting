		...
		for ( $j = 0; $j < count( $auth_methods[ $i ] ); $j ++ ) {
         ...
         //  					$form .='   <span style="float:right">
			...
			if ( $is_auth_method_av && $category == 'free_plan' ) {
				...
				if($auth_method_abr == 'OTPOverEmail')
				{

					$is_auth_method_configured = $Mo2fdbQueries->get_user_detail( 'user_registration_with_miniorange', $user->ID );
					if($is_auth_method_configured == "SUCCESS")
						$is_auth_method_configured = true;
...
//						$twofactor_registered = $configured_auth_method;
