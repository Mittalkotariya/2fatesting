...
class Miniorange_Authentication {
    ...
    //             $array_dissmised_pointers = array_diff($array_dissmised_pointers,mo_saml_options_enum_pointersMo2FA::$DEFAULT);
    //             update_user_meta($uid,'mo2f_dismissed_wp_pointers',implode(",",$array_dissmised_pointers));
    //             update_option('mo2f_plugin_wise_tour_initiated',true);
...
//						    update_user_meta( $user->ID, 'currentMethod', $current_method );
			...
			$cloud_method = array('miniOrangeQRCodeAuthentication','miniOrangePushNotification','miniOrangeSoftToken');

//			if(MO2F_IS_ONPREM and  $configuredMethod =='EmailVerification' )
//			{
//				update_user_meta($user->ID,'currentMethod','Email Verification');
//				mo2f_display_test_2fa_notification($user);
//			}else if($selectedAction == 'select2factor' and MO2F_IS_ONPREM and !in_array($configuredMethod,$cloud_method))
//			{
			...
			if ( $is_customer_registered ) {
				...
				if ( $selected_action == "select2factor" ) {
					...
					if ( $selected_2FA_method == 'OTP Over SMS' && $user_phone == 'false' ) {
						...
						{	
							//$enduser->send_otp_token($email,'EMAIL',$customer_key,$api_key);
							...
							//update_option("mo2f_current_method","OTPOverEmail");
							//update_user_meta($user->ID,'currentMethod',$selected_2FA_method);
						...
						}else if($selected_2FA_method == "Email Verification")
						{
							...
							//update_option("mo2f_current_method","EmailVerification");
						}
			...
			if ( ! wp_verify_nonce( $nonce, 'mo-2factor-test-authentication-method-nonce' ) ) {
				...
				if ( $selected_2FA_method == 'Security Questions' ) {
					...
					if($selected_2FA_method == 'OTP Over Email')	
					{
						...
						//$selected_2FA_method_server = 'EMAIL';
