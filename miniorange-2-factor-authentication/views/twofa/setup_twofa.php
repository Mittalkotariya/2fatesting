  ...
  -->       <script>
     ...
     //        cosole.log('saasdsa');
            function configureOrSet2ndFactor_free_plan(authMethod, action, cloudswitch=null,allowed=null) {
                ...
                // if(is_onprem ==0 && allowed != 1 )
                // {
                //     jQuery('#mo_scan_message').empty();
                //     jQuery('#mo_scan_message').append("<div id='notice_div' class='overlay_error'><div class='popup_text'>&nbsp&nbsp <b>Only one user is available in the free plan. <a href='<?php echo $upgrade_url; ?>' style='color: #2980b9'>Add more users</a> ");
                //     window.onload =  nav_popup();
                    
                //     return ;    
                // }
                if (cloudswitch != null && cloudswitch != 0) 
                {
                    var nonce = '<?php echo wp_create_nonce("singleUserNonce");?>';
                     var data = {
                                'action'                    : 'mo_two_factor_ajax',
                ...
                else
                {
                    ...
                    {   
                        ...
                        //var is_registered = '<?php //echo get_option("mo2f_miniorange_admin");?>';
                        
                        if(authMethod == 'miniOrangeQRCodeAuthentication' || authMethod == 'miniOrangeSoftToken' || authMethod == 'miniOrangePushNotification' || authMethod =='OTPOverSMS' || authMethod =='OTPOverEmail' )
                        {
                          //  if(is_registered)
                            //{
                                // document.getElementById('mo2f_cloud').style.display = "block";
                                // document.getElementById('mo2f_cloud_modal').style.display = "block";
                            //}
                            // else
                            // {
                                jQuery('#mo2f_save_free_plan_auth_methods_form').submit();
